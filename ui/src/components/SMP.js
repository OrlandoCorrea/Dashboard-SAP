...
class SMP extends Component {
  ...
  componentWillMount() {
    this.setState({ state:this.props.states,rules:this.props.rules,alerts: this.props.alerts,smpData: this.props.data});
  }

  componentWillReceiveProps(nextProps) {
      if(this.props != nextProps) {
        this.setState({ state:nextProps.states,rules:nextProps.rules,alerts: nextProps.alerts,smpData: nextProps.data});
      }
  }
  handleClick(e) {
  ...
  render(){
    ...
    /// SMP
    var d_sm51_smp = []
    if (this.state.smpData.smpSm51.length>0) {
      let icon = ''
        if (this.state.state.smpsm51_alert) {
        ...
        <div className="widget-summary widget-summary-sm">
          ...
          <div className="widget-summary-col">
            <div className="summary">
              <h5 className="title">Versión {this.state.smpData.smpSm51[0].kernel}</h5>
              <h5 className="title">Parche {this.state.smpData.smpSm51[0].patch}</h5>
              <div className="info">
                <strong className="amount">{this.state.smpData.smpSm51[0].sid}</strong>
                <p>{this.state.smpData.smpSm51[0].system} {this.state.smpData.smpSm51[0].environment}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    //SICK
    var d_sick = []
    if (this.state.smpData.smpSick.length>0) {
      var icon = ''
      if (this.state.smpData.smpSick[0].status==='OK') {
        icon = <div className="summary-icon bg-success">
              <i className="fa fa-check"></i>
            </div>
      }
      ...
      var response = this.state.smpData.smpSick[0].response
      ...
      d_sick.push(<div className="widget-summary widget-summary-sm">
        ...
        <div className="widget-summary-col">
          <div className="summary">
            ...
            <div className="info">
              <strong className="amount">{this.state.smpData.smpSick[0].sid}</strong>
              <p>{this.state.smpData.smpSick[0].system+' '+this.state.smpData.smpSick[0].environment}</p>
            </div>
          </div>
        </div>
      </div>)
    ...
    var borderColorSmlg = []
    let flag_smlg = false
    if (this.state.smpData.smpSmlg.length>0) {
      for (var i = 0; i < this.state.smpData.smpSmlg.length; i++) {
        d_smlg.push(parseInt(this.state.smpData.smpSmlg[i].response_time))
        l_smlg.push(this.state.smpData.smpSmlg[i].instance)
        if (this.state.state.array_smpsmlg_alert.length > 0) {
           for (let index = 0; index < this.state.state.array_smpsmlg_alert.length; index++) {
             const element = this.state.state.array_smpsmlg_alert[index];
             if (element === this.state.smpData.smpSmlg[i].instance) {
               flag_smlg = true
             }
           }
           if (flag_smlg) {
    ...
    //SM50
    var ncsol013_dialog_data = [], ncsol013_update_data = [],  ncsol013_background_data = [], ncsol013_spool_data = []

    if (this.state.smpData.smpSm50_all.length>0) {
        console.log(this.state.smpData.smpSm50_all);
      for (var i = 0; i < this.state.smpData.smpSm50_all.length; i++) {
        if (this.state.smpData.smpSm50_all[i].server === 'ncsol013_SMP_00') {
          ncsol013_dialog_data.push(parseInt(this.state.smpData.smpSm50_all[i].dialog.split('(')[0].split('/')[0]) - parseInt(this.state.smpData.smpSm50_all[i].dialog.split('(')[0].split('/')[1]))
          ncsol013_dialog_data.push(parseInt(this.state.smpData.smpSm50_all[i].dialog.split('(')[0].split('/')[1]))
          ncsol013_update_data.push(parseInt(this.state.smpData.smpSm50_all[i].update.split('(')[0].split('/')[0]) - parseInt(this.state.smpData.smpSm50_all[i].update.split('(')[0].split('/')[1]))
          ncsol013_update_data.push(parseInt(this.state.smpData.smpSm50_all[i].update.split('(')[0].split('/')[1]))
          ncsol013_background_data.push(parseInt(this.state.smpData.smpSm50_all[i].background.split('(')[0].split('/')[0]) - parseInt(this.state.smpData.smpSm50_all[i].background.split('(')[0].split('/')[1]))
          ncsol013_background_data.push(parseInt(this.state.smpData.smpSm50_all[i].background.split('(')[0].split('/')[1]))
          ncsol013_spool_data.push(parseInt(this.state.smpData.smpSm50_all[i].spool.split('(')[0].split('/')[0]) - parseInt(this.state.smpData.smpSm50_all[i].spool.split('(')[0].split('/')[1]))
          ncsol013_spool_data.push(parseInt(this.state.smpData.smpSm50_all[i].spool.split('(')[0].split('/')[1]))
        }
      }
    }
    ...
    var ncsol013_background = {
        datasets: [{
            data: ncsol013_background_data,
            backgroundColor: [
              'rgba(54, 162, 235, 0.2)',
              'rgba(135, 189, 53, 0.2)'
    ...
    if (this.state.smpData.smpSm66.length>0) {
      for (var i = 0; i < this.state.smpData.smpSm66.length; i++) {
        const element = this.state.smpData.smpSm66[i];
        if (element.type == "DIA") {
          array_sm66_dia.push({
            server: this.state.smpData.smpSm66[i].server,
            client: this.state.smpData.smpSm66[i].user_name,
            time: this.state.smpData.smpSm66[i].time,
            state: this.state.smpData.smpSm66[i].status
          })
        }
        ...
        <div className="col-md-12">
          ...
          getTrProps={(state, rowInfo, column) => {
            if (rowInfo) {
              return {
                style: {
                  background: rowInfo.row.time > this.state.state.smpSm66Rule_dia ? '#ff8080' : null
                }
              }
            } else {
              return {
        ...
        <div className="col-md-12">
          ...
          getTrProps={(state, rowInfo, column) => {
            if (rowInfo) {
              return {
                style: {
                  background: rowInfo.row.time > this.state.state.smpSm66Rule_btc ? '#ff8080' : null
                }
              }
            } else {
              return {
        ...
        <div className="col-md-12">
          ...
          getTrProps={(state, rowInfo, column) => {
            if (rowInfo) {
              return {
                style: {
                  background: rowInfo.row.time > this.state.state.smpSm66Rule_upd ? '#ff8080' : null
                }
              }
            } else {
              return {
    ...
    var st22_labels = []
    var st22_data = []
    for (var i = 0; i < this.state.smpData.smpSt22Top.length; i++) {
      if (this.state.smpData.smpSt22Top[i]) {
        st22_labels.push(this.state.smpData.smpSt22Top[i].dump)
        st22_data.push(this.state.smpData.smpSt22Top[i].number)
      }
    }
    var dataSt22 = {
        labels: st22_labels,
    ...
    if (this.state.smpData.smpSm12.length>0) {
      for (var i = 0; i < this.state.smpData.smpSm12.length; i++) {
        if (this.state.smpData.smpSm12[i].lock_model == "E") {
          array_sm12E.push({
            client: this.state.smpData.smpSm12[i].client,
            username: this.state.smpData.smpSm12[i].username,
            time: this.state.smpData.smpSm12[i].time,
            table_name: this.state.smpData.smpSm12[i].table_name,
            lock_argument: this.state.smpData.smpSm12[i].lock_argument
          })
        }
        else if (this.state.smpData.smpSm12[i].lock_model == "S") {
          array_sm12S.push({
            client: this.state.smpData.smpSm12[i].client,
            username: this.state.smpData.smpSm12[i].username,
            time: this.state.smpData.smpSm12[i].time,
        ...
        else if (this.state.smpData.smpSm12[i].lock_model == "X") {
          array_sm12X.push({
            client: this.state.smpData.smpSm12[i].client,
            username: this.state.smpData.smpSm12[i].username,
            time: this.state.smpData.smpSm12[i].time,
        ...
        else if (this.state.smpData.smpSm12[i].lock_model == "O") {
          array_sm12O.push({
            client: this.state.smpData.smpSm12[i].client,
            username: this.state.smpData.smpSm12[i].username,
            time: this.state.smpData.smpSm12[i].time,
    ...
    if (this.state.smpData.smpSm13.length>0) {
      for (var i = 0; i < this.state.smpData.smpSm13.length; i++) {
        if (this.state.smpData.smpSm13[i].status == "Initial") {
          array_sm13_update.push({
            client: this.state.smpData.smpSm13[i].client,
            username: this.state.smpData.smpSm13[i].username,
            time: this.state.smpData.smpSm13[i].time,
            tcode: this.state.smpData.smpSm13[i].tcode,
            status: this.state.smpData.smpSm13[i].status
          })
        }
        else if (this.state.smpData.smpSm13[i].status == "Error(noretry)" || this.state.smpData.smpSm13[i].status == "Error") {
        ...
        else if (this.state.smpData.smpSm13[i].status == "V1processed") {
        ...
        else if (this.state.smpData.smpSm13[i].status == "V2processed") {
    ...
    if (this.state.smpData.smpSm37.length>0) {
      for (var i = 0; i < this.state.smpData.smpSm37.length; i++) {
        if (this.state.smpData.smpSm37[i].status === 'Ready') {
          array_sm37_ready.push({
            job: this.state.smpData.smpSm37[i].job,
            jobcreated: this.state.smpData.smpSm37[i].jobcreated,
            start_date: this.state.smpData.smpSm37[i].start_date,
            start_time: this.state.smpData.smpSm37[i].start_time,
            Duration_sec: this.state.smpData.smpSm37[i].Duration_sec,
            Delay_sec: this.state.smpData.smpSm37[i].Delay_sec
          })
        }
        else if (this.state.smpData.smpSm37[i].status == "Active") {
        ...
        if (this.state.smpData.smpSm37[i].status === 'Canceled') {
    ...
    //SMQ2
    var smq2_data = []
    if (this.state.smpData.smpSmq2.length>0) {
      smq2_data.push(parseInt(this.state.smpData.smpSmq2[0].Entries))
    }
    var dataSmq2 = {
				labels: ["SMP"],
				datasets: [{
					...
					<div id="userbox" className="userbox">
						<a  data-toggle="dropdown">
							<figure className="profile-picture">
								<img src="https://pbs.twimg.com/profile_images/786223609341743104/na3WJJlB_400x400.jpg" className="img-circle" data-lock-picture="assets/images/!logged-user.jpg" />
							</figure>
			...
			<div className="inner-wrapper">
				...
				<section role="main" className="content-body">
					<header className="page-header">
						<h2>Evaluación Técnica SAP - SMP</h2>
						<div className="right-wrapper pull-right">
							<ol className="breadcrumbs">
								...
								<li><span>Evaluación Técnica SAP - SMP</span></li>
							</ol>
                  ...
                  <div className="col-md-12">
                    <div className="tabs">
                      <ul className="nav nav-tabs nav-justified">
                        <li className="active">
                          <a href="#ncsol013" data-toggle="tab" className="text-center">ncsol013_SMP_00</a>
                        </li>
                      </ul>
                      <div className="tab-content">
                        <div id="ncsol013" className="tab-pane active">
                          <div className="row">
                            <div className="col-md-3">
                              <Doughnut ref='chart' width={200} height={200} data={ncsol013_dialog} />
                              <h6 className="text-center">Dialog</h6>
                              <p className="text-center">Total: {ncsol013_dialog_data[0]+ncsol013_dialog_data[1]}</p>
                            </div>
                            <div className="col-md-3">
                              <Doughnut ref='chart' width={200} height={200} data={ncsol013_update} />
                              <h6 className="text-center">Update</h6>
                              <p className="text-center">Total: {ncsol013_update_data[0]+ncsol013_update_data[1]}</p>
                            </div>
                            <div className="col-md-3">
                              <Doughnut ref='chart' width={200} height={200} data={ncsol013_background} />
                              <h6 className="text-center">Background</h6>
                              <p className="text-center">Total: {ncsol013_background_data[0]+ncsol013_background_data[1]}</p>
                            </div>
                            <div className="col-md-3">
                              <Doughnut ref='chart' width={200} height={200} data={ncsol013_spool} />
                              <h6 className="text-center">Spool</h6>
                              <p className="text-center">Total: {ncsol013_spool_data[0]+ncsol013_spool_data[1]}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <span className="pull-right">Last Update : {this.props.data.datetime}</span>
                    ...
                    <div className="col-md-1"></div>
										<div className="col-md-10">
											...
											<h6 className="text-center">SMP</h6>
										</div>
